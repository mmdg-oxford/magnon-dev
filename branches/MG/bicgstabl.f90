...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  !Vol. 13, No. 2, pp. 631-644
    ...
    !           END IF
     ...
     do iterj = 1, lmresloc
        do ibnd = 1, nbnd
                rho     = ZDOTC (ndmx*npol, gt(1,ibnd), 1, g(1,ibnd), 1)
      ...
      !"get out if all bands are converged."
            conv_root = .true.
            do ibnd = 1, nbnd
               anorm = sqrt ( abs ( ZDOTC (ndmx*npol, g(1,ibnd), 1, g(1,ibnd), 1)  ) )
     ...
     ! MA x
        ...
        ! c(ibnd) = ZDOTC (ndim, hn(1,ibnd), 1, gt(1,ibnd), 1)
                c(ibnd) = ZDOTC (ndmx*npol, gt(1,ibnd), 1, hn(1,ibnd), 1)
   ...
   do ibnd = 1, nbnd
     do iterj = 2, lmresloc + 1
        if (lmresloc.gt.1)  then
            do iteri = 2, iterj
               tau(iteri, iterj) = (1.0d0/sigma(iteri))*ZDOTC(ndmx*npol, g(1,iterj), g(1,iteri), 1)
...
!       gmgammap(iterj) = (1.0d0/sigma(iterj)) * ZDOTC(ndim, g(1,ibnd), 1, t(1,ibnd), 1)
        gmgammap(iterj) = (1.0d0/sigma(iterj)) * ZDOTC(ndmx*npol, t(1,ibnd), 1, g(1,ibnd), 1)
     enddo
   enddo
...
!      x  = x  + gmgamma(1)  * \hat{r}_{0}
       call ZAXPY (ndmx*npol,  (gmgamma(lmresloc+1)), g(1,ibnd),  1, dpsi(1,ibnd), 1)
...
!     \hat{r}_{0}  = \hat{r}_{0}  - gammap_{l}*r_{lmresloc}
       call ZAXPY (ndmx*npol, -(gmgammap(lmresloc+1)), t(1,ibnd), 1, g(1,ibnd), 1)
